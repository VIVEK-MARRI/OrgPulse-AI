        ...
        {decisions.length === 0 ? (
          ...
          decisions.map(decision => (
            <div className="decision-item" key={decision.id}>
              <div className="decision-timeline-dot" style={decisionDotStyle(decision.type)} />
              <div className="decision-content">
                ...
                <div className="decision-meta">
                  ...
                  {decision.made_by && <span className="text-sm">by <strong>{decision.made_by}</strong></span>}
                  <span className="badge badge-neutral font-mono text-xs">{decision.id}</span>
                </div>
                {decision.rationale && <div className="decision-rationale">Rationale: {decision.rationale}</div>}
                {decision.source_quote && <div className="source-quote">"{decision.source_quote}"</div>}
              </div>
            </div>
          ))
        )}
